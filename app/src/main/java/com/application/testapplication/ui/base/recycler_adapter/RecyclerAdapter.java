...
public class RecyclerAdapter extends RecyclerView.Adapter<RecyclerAdapterViewHolder>
    ...
    public void setItemsList(List<AdapterItem> itemsList) {
...
//            View view = item.inflateItemView(ctx , parent);
    ...
    public void onBindViewHolder(RecyclerAdapterViewHolder holder, int position) {
        Object data = null;
        if (adapterIsEmptyAndEmptyItemIsDefined()) {
            data = emptyItem.getData();
            emptyItem.bindData(holder, emptyItem.getData(), position);
        } else {
            data = getItems().get(position).getData();
            getItems().get(position).bindData(holder, getItems().get(position).getData(), position);
        }
        holder.bind(data);
    }
    ...
    protected boolean adapterIsEmptyAndEmptyItemIsDefined() {
