...
public abstract class AdapterItem<T extends RecyclerAdapterViewHolder> implements Comparable<AdapterItem> {
    ...
    public Resources getResources() {
        return MIVIApplication.getApplicationInstance().getResources();
    }

    public Context getAppContext() {
        return MIVIApplication.getApplicationInstance().getApplicationContext();
    }
    ...
    T getViewHolder(ViewDataBinding view, RecyclerAdapterNotifier adapter)
        ...
        for (Class cl : getClass().getClasses()) {
            if (RecyclerAdapterViewHolder.class.isAssignableFrom(cl)) {
                ...
                return clazz.getConstructor(ViewDataBinding.class, RecyclerAdapterNotifier.class)
     ...
     */
    protected abstract void bindData(T holder, Object data, int position);
    ...
    protected abstract void onViewRecycled(T holder);
}
